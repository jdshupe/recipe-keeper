 ...
 * Handles both string format and structured ingredient objects
 */
function parseIngredientString(ingredientInput) {
  // If already a structured ingredient object, return its parsed fields
  if (ingredientInput && typeof ingredientInput === 'object') {
    return {
      quantity: ingredientInput.quantity || null,
      unit: ingredientInput.unit || null,
      name: ingredientInput.name || ingredientInput.originalText || '',
      preparation: ingredientInput.preparation || null,
      original: ingredientInput.originalText || ''
    };
  }
  
  // Handle non-string inputs
  if (typeof ingredientInput !== 'string') {
    return { quantity: null, unit: null, name: String(ingredientInput || ''), preparation: null, original: '' };
  }
...
/**
 * Common ingredient word variations for fuzzy matching
 * Maps variations/modifiers to their base ingredient
 */
...
/**
 * Match a parsed ingredient to the database
 * @param {Object} parsedIngredient - Object from parseIngredientString with at least { name }
 * @returns {Promise<{ingredient: Object|null, confidence: string}>}
 */
async function matchIngredientToDatabase(parsedIngredient) {
  // Handle null/undefined/empty input
  if (!parsedIngredient || !parsedIngredient.name) {
    return { ingredient: null, confidence: 'none' };
  }
  
  const ingredientName = parsedIngredient.name.toLowerCase().trim();
  ...
  const exactMatch = await findIngredientByName(ingredientName);
  if (exactMatch) {
    return { ingredient: exactMatch, confidence: 'exact' };
  }
  
  // 2. Try matching via known variations
  const variationBase = INGREDIENT_VARIATIONS[ingredientName];
  if (variationBase) {
    const variationMatch = await findIngredientByName(variationBase);
    if (variationMatch) {
      return { ingredient: variationMatch, confidence: 'high' };
    }
  }
  
  // 3. Try fuzzy matching - starts with
  const startsWithMatch = allIngredients.find(ing => {
    const ingName = ing.name.toLowerCase();
    return ingName.startsWith(ingredientName) || ingredientName.startsWith(ingName);
  });
  if (startsWithMatch) {
    return { ingredient: startsWithMatch, confidence: 'high' };
  }
  
  // 4. Try fuzzy matching - contains
  const containsMatch = allIngredients.find(ing => {
    const ingName = ing.name.toLowerCase();
    return ingName.includes(ingredientName) || ingredientName.includes(ingName);
  });
  if (containsMatch) {
    return { ingredient: containsMatch, confidence: 'medium' };
  }
  
  // 5. Try word-by-word matching (e.g., "chicken breast skinless" should match "chicken")
  const words = ingredientName.split(/\s+/);
  for (const word of words) {
    if (word.length < 3) continue; // Skip short words like "of", "a", etc.
    
    const wordMatch = allIngredients.find(ing => {
      const ingName = ing.name.toLowerCase();
      return ingName === word || 
             ingName.includes(word) ||
             (ing.aliases && ing.aliases.some(a => a.toLowerCase() === word));
    });
    if (wordMatch) {
      return { ingredient: wordMatch, confidence: 'low' };
    }
  }
  
  // 6. Check if any variation base word matches
  for (const [variation, base] of Object.entries(INGREDIENT_VARIATIONS)) {
    if (ingredientName.includes(variation) || variation.includes(ingredientName)) {
      const baseMatch = await findIngredientByName(base);
      if (baseMatch) {
        return { ingredient: baseMatch, confidence: 'low' };
      }
    }
  }
...
/**
 * Parse an ingredient string and match it to the database
 * Combines parseIngredientString with matchIngredientToDatabase
 * @param {string} ingredientString - Raw ingredient string like "2 cups all-purpose flour"
 * @returns {Promise<Object>} Structured ingredient object ready for recipe storage
 */
async function parseAndMatchIngredient(ingredientString) {
  // Handle null/undefined/empty input
  ...
  // Match to database
  const { ingredient, confidence } = await matchIngredientToDatabase(parsed);
...
/**
 * Parse and match an array of ingredient strings
 * @param {string[]} ingredientStrings - Array of raw ingredient strings
 * @returns {Promise<Object[]>} Array of structured ingredient objects
 */
async function parseAndMatchIngredients(ingredientStrings) {
  // Handle null/undefined/empty input
  if (!ingredientStrings || !Array.isArray(ingredientStrings)) {
    return [];
  }
  
  // Filter out null/undefined/empty strings and process in parallel
  const validStrings = ingredientStrings.filter(s => s && typeof s === 'string' && s.trim());
  
  const results = await Promise.all(
    validStrings.map(str => parseAndMatchIngredient(str))
  );
 ...
 * Returns complete ingredient object including all metadata
 * @param {string} ingredientId - The ingredient ID
 * @returns {Promise<Object|null>} Full ingredient details or null if not found
 */
async function getIngredientDetails(ingredientId) {
  ...
  const ingredient = await getIngredientById(trimmedId);
  
  if (!ingredient) {
    return null;
  }
  
  // Return the full ingredient with additional computed properties
  return {
    ...ingredient,
    // Add category info if available
    categoryInfo: ingredient.category ? INGREDIENT_CATEGORIES[ingredient.category] : null,
    // Add shelf life info if available
    shelfLife: INGREDIENT_SHELF_LIFE[ingredient.name.toLowerCase()] || 
               (ingredient.category ? SHELF_LIFE_DEFAULTS[ingredient.category] : null),
    // Add available substitutes
    substitutes: SUBSTITUTIONS[ingredient.name.toLowerCase()] || []
  };
}
...
module.exports = {
  ...
  // Ingredient matching
  matchIngredientToDatabase,
  parseAndMatchIngredient,
  parseAndMatchIngredients,
