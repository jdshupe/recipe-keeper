...
const { parseAndMatchIngredients } = require('./ingredients');
...
async function scrapeRecipe(url, options = {}) {
  const { structuredIngredients = true } = options;
  ...
  const scrapedIngredients = (schemaRecipe.recipeIngredient || []).map(ing => String(ing).trim());
  
  // Convert to structured ingredients if enabled
  let ingredients = scrapedIngredients;
  if (structuredIngredients && scrapedIngredients.length > 0) {
    try {
      ingredients = await parseAndMatchIngredients(scrapedIngredients);
    } catch (err) {
      // Fall back to plain strings if matching fails
      console.warn('Ingredient matching failed, using plain strings:', err.message);
  ...
  // Flatten tags - recipeCategory and recipeCuisine can be strings or arrays
  const rawTags = [schemaRecipe.recipeCategory, schemaRecipe.recipeCuisine]
    .flat()  // Flatten in case they're arrays
    .filter(tag => tag && typeof tag === 'string');  // Only keep valid strings
  ...
  return {
    ...
    tags: rawTags,
    ingredients,
    originalIngredients: scrapedIngredients,
    instructions,
    dateAdded: new Date().toISOString()
  };
}
